...
fs.copyFileSync(path.join(ROOT, 'references', 'examples', 'mysc-2026', 'assets', 'mysc-logo.png'),
  path.join(TMP, 'imgs', 'a.png'));
...
/* ── 4.8.1 — 개발환경 통제: 저장소의 테스트·예제에 실제 개인정보가 없다 ── */
{
  ...
  const pat = /\d{6}-[1-4]\d{6}|01[016-9]-?\d{3,4}-?\d{4}|[\w.]+@(?!example\.|mysc\.co\.kr|anthropic\.com)[\w-]+\.(com|net|kr)/;
  const hits = [];
