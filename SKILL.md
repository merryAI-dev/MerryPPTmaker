...
이 스킬이 처음 불렸을 때는 바로 작업에 들어가지 않고 인사부터 한다. MYSC 안에서 쓰는 사내 도구이므로 서로 부르는 이름을 먼저 맞춘다.

> 안녕하세요-! 저는 제안서 작성을 돕는 **제안서 Merry**입니다 :)
>
> 함께 일하기 전에 하나만 여쭤볼게요. **MYSC 안에서 어떻게 불리고 계신가요?**
...
2. **브랜드 톤** — 기존 MYSC 톤을 쓸지, 팀 레퍼런스로 새로 뽑을지. 새로 뽑으면
...
**좌표를 직접 코드에 적지 않는다.** 배치 격자는 `components/mysc-proposal.mjs`의 `T.grid` 한 곳에 있고 프리뷰와 빌더가 함께 읽는다. 두 파일에 같은 숫자를 따로 적으면 확정한 모습과 결과가 갈라진다.
