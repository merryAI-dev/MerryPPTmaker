...
레퍼런스가 있는 덱의 Stage 1은 `references/design-token-extraction.md`도 읽는다. 이 파일은 상단 rule 분할, 좌측 rail, 표 grid, 차트 axis/series처럼 작은 반복 토큰을 강제로 추출하는 기준이다.
...
- observed design token table: canvas, header, rail, typography, palette, table, chart, diagram, footer, forbidden
- forbidden patterns: 레퍼런스에 없는 장식/카드/색상/아이콘 스타일
- body slide density rule: 한 장에 담을 수 있는 최대 표 행, bullet 수, chart 수

토큰 추출에는 토큰을 많이 써도 된다. 오히려 레퍼런스가 보고서형이거나 표/차트가 많은 경우 토큰을 적게 쓰는 것이 실패다. “청록/회색 톤”처럼 요약하지 말고 `header.top_rule.primary_segment`, `header.top_rule.secondary_segment`, `table.border`, `chart.grid`, `chart.primary_series`처럼 역할 기반 이름으로 쓴다.
...
- 상단 rule 분할, 좌측 rail, 표 header/grid, 차트 grid/axis/series가 보이면 반드시 별도 토큰으로 기록한다.
...
- 상단 rule 분할, 좌측 rail, 표/차트 색상 역할 같은 핵심 token이 누락되면 재생성한다.
...
네이티브 PPT source script는 Stage 1 design tokens를 상수로 옮긴다. split top rule은 두 개 이상의 선/사각형 segment로 구현하고, 표/차트는 tokenized border/grid/series 색상을 사용한다.
