...
레퍼런스는 “청록/회색 톤”처럼 요약하지 않습니다. 실제 반복되는 작은 시각 토큰까지 기록해야 합니다.

- 상단 rule이 연두/녹색 구간과 회색 구간으로 나뉘는지
- 좌측 rail이 primary bar와 neutral bar로 구성되는지
...
| --- | --- | --- | --- | --- |
| `header.top_rule.primary_segment` | green/teal segment, approx 35-50% | body page top header | body slide 상단 rule의 앞 구간 | medium |
| `header.top_rule.secondary_segment` | light gray segment, remaining width | body page top header | primary segment 뒤에서 우측 margin까지 | medium |
| `table.border` | pale gray thin grid | body table | 모든 표 border와 chart grid 후보 | high |
| `chart.primary_series` | green/teal line or bar | body chart | primary metric에만 사용 | medium |

필수 token group:

- Canvas: background, aspect ratio, outer margin, safe area
- Header: title position, top rule thickness, split ratio, primary/secondary segment color
- Rail: left rail width, primary/neutral segment color
...
- split line, split rail, alternating row, chart grid처럼 반복되는 요소는 반드시 token으로 기록합니다.
...
- 레퍼런스에 상단 rule/좌측 rail/표/차트가 보이는데 해당 token이 없습니다.
- palette가 “green/gray”처럼 역할 없이 요약되어 있습니다.
...
레퍼런스의 시각 시스템을 role-based token으로 추출하는 기준입니다. 상단 rule 분할, 좌측 rail, 표 grid, 차트 grid/axis/series 같은 작은 반복 요소를 반드시 기록하게 만듭니다.
...
- split top rule은 하나의 선이 아니라 token에 맞춘 segment로 구현
...
```text
https://github.com/merryAI-dev/MerryPPTmaker
```

## Commit Policy

이 저장소에는 스킬 소스와 문서만 커밋합니다.

커밋하지 않는 것:

- 생성된 PPTX
- 생성된 PNG slide images
- `vendor/`
- 임시 작업 디렉터리
- 사용자 원본 자료
- 비공개 레퍼런스 파일
