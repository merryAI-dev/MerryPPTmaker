...
레퍼런스를 “청록/회색 보고서 톤”처럼 요약하면 실제 디자인 시스템이 사라진다. 보고서형 레퍼런스는 작은 반복 토큰으로 구성된다.

- 상단 rule이 한 색이 아니라 연두/녹색 구간과 회색 구간으로 분할됨
- 좌측 rail이 단일 막대가 아니라 primary bar와 neutral bar 조합으로 반복됨
- 표 header, border, row fill, chart axis, chart grid가 같은 회색/녹색 체계를 공유함
...
| Header | title position, top rule thickness, top rule split ratio, primary segment color, secondary segment color |
| Rail | left rail width, rail segment colors, rail relationship to header/body/footer |
...
- `token`: stable name such as `header.top_rule.primary_segment`
...
| --- | --- | --- | --- | --- |
| header.top_rule.primary_segment | green/teal segment, approx 35-50% of rule | body page top header | Use before gray segment on body slides | medium |
| header.top_rule.secondary_segment | light gray segment, remaining width | body page top header | Continue from primary segment to right margin | medium |
| table.border | pale gray, thin line | body table grid | Use for all table borders and chart grid lines | high |
| chart.primary_series | green/teal line or bar | body chart | Use for primary data series only | medium |

## Extraction Rules

- Prefer observed ratios over vague adjectives. Write “primary segment about half of top rule” instead of “nice accent line”.
...
- If the reference uses split lines, split rails, alternating rows, or repeated grid colors, record those as tokens.
...
- “Use `header.top_rule.primary_segment` followed by `header.top_rule.secondary_segment`.”
...
- Implement split top rules as separate line or rectangle segments.
...
- top rule, rail, table, or chart tokens are missing when the reference visibly contains those elements.
- palette is only summarized as “green/gray” without role-based tokens.
