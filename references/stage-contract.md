...
레퍼런스에 상단 rule, 좌측 rail, 표, 차트가 보이는데 `DESIGN.md`에 해당 token table이 없으면 Stage 1은 실패다. “green/gray report style” 같은 요약은 완료 조건이 아니다.
...
- token evidence: top rule, rail, table grid, chart grid/axis/series, footer rule
- 접근 가능 여부
...
- split top rule, green/gray table treatment, gray chart grid, green primary series가 보이면 반드시 별도 token으로 분리한다.
