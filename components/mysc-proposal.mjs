/**
 * MYSC 제안서 톤 네이티브 PPTX 컴포넌트.
 *
 * 모든 좌표/색상/크기는 `references/examples/mysc-2026/tokens.md`에서 관찰된 값이다.
 * 추측한 값은 없다. 원본 좌표는 `references/examples/mysc-2026/shapes-full.json` 참고.
 *
 * 캔버스는 16:9가 아니라 A4 landscape(11.693 x 8.267in)다. 반드시 defineLayout으로
 * 등록하고 써야 한다. `applyLayout(pptx)`가 그 일을 한다.
 *
 * 사용 예:
 *   import PptxGenJS from 'pptxgenjs';
 *   import { applyLayout, coverSlide, bodySlide, statGrid } from './mysc-proposal.mjs';
 *   const pptx = new PptxGenJS();
 *   applyLayout(pptx);
 *   coverSlide(pptx, { year: '2026', title: 'EMA\n액셀러레이팅 프로그램\n제안서' });
 */
...
export const ASSET_DIR = path.join(HERE, '..', 'references', 'examples', 'mysc-2026', 'assets');
...
export const T = {
  ...
  color: {
    navy: '0C2044',          // 주 브랜드 네이비 (텍스트 92회 / fill 44회)
    navyDeep: '001D45',
    navyMid: '01397E',       // 리드문 구분자, 소형 마커
    cyan: '59C2E2',          // accent - 구조 화살표, 강조
    cyanLabel: '5BBEDE',     // 리드문 라벨
    cyanTintStrong: 'BEE6FB',// 태그/카드 배경
    cyanTint: 'BAE8FB',      // 표 헤더 fill
    yellow: 'FADE4B',        // 드문 하이라이트
    gray: '7F7F7F',          // 보조 설명
    white: 'FFFFFF',
    rule: '001521',          // 헤더 구분선
    headerBandFrom: '091823',// 헤더 밴드 그라디언트 시작
    headerBandTo: '0C2044',
  },

  font: {
    // 원본은 Pretendard 전량. Pretendard 미설치 환경에서는 PowerPoint가 대체하므로
    // 배포 전 대상 PC에 Pretendard가 있는지 확인할 것.
    family: 'Pretendard',
    coverYear: 48,      // Light
  ...
  grid: {
    ...
    natural: { bulletRow: 0.36, bulletPad: 0.12, flow: 2.4, steps: 1.5, stats: 1.35 },
...
export function applyLayout(pptx) {
  pptx.defineLayout({ name: 'MYSC_A4', width: T.canvas.w, height: T.canvas.h });
  pptx.layout = 'MYSC_A4';
...
/**
 * 표. 헤더 fill은 관찰된 옅은 하늘색(BAE8FB), 본문은 밀도 높은 9.7pt.
...
/**
 * 프로세스 chevron 밴드 (homePlate). 원본 slide 27에서 cyan(59C2E2)으로 관찰.
 */
