 ...
 * 읽는 tokens.md로만 남고, 빌더는 여전히 MYSC 하드코딩을 쓰기 때문이다.
 * 여기서 그 간극을 잇는다: --brand <이름|경로> → brand.json → T 덮어쓰기.
 *
 * 원칙:
 * - MYSC T를 "구조의 원본"으로 삼는다. 검증된 배치 비율을 새 캔버스에 스케일한다.
 * - brand.json에 실측값이 있으면(헤더 룰, 콘텐츠 경계, 표 행높이) 스케일값보다 우선한다.
 * - 실측이 없는 항목은 지어내지 않고 MYSC 비율을 따른다. 온보딩 화면에서 사람이
...
import { T } from './mysc-proposal.mjs';

const SELF_DIR = path.dirname(fileURLToPath(import.meta.url));
const SKILL_DIR = path.resolve(SELF_DIR, '..');

/** MYSC 캔버스. 모든 기존 좌표의 기준이므로 T가 바뀌기 전에 고정해 둔다. */
...
/** 팔레트에서 역할 색을 뽑는다. 못 찾은 역할은 MYSC 색을 유지하지 않고 primary 계열로 통일한다. */
...
export function applyBrand(nameOrPath) {
  ...
  Object.assign(T, {
    ...
    grid: layout.grid,
    stat: layout.stat,
    // 표지/간지: 브랜드 팩에는 MYSC 그라디언트 이미지가 없다.
    // 컴포넌트가 이 플래그를 보고 팔레트 기반으로 그린다.
